...
impl Lang {
    ...
    pub(crate) fn t<'a>(&self, key: &'a str) -> &'a str {
        match (self, key) {
            ...
            (Lang::En, "opt_alias") => "✏️  Set device alias",
            (Lang::Zh, "opt_alias") => "✏️  设置设备昵称",
            (Lang::En, "prmpt_alias") => "Enter a new alias (empty to clear):",
            (Lang::Zh, "prmpt_alias") => "请输入新昵称（直接回车清除）:",
            (Lang::En, "alias_help") => "Alias becomes yundrone-<alias>-<mac6>. Empty or bleinit restores the default name.",
            (Lang::Zh, "alias_help") => "昵称会变成 yundrone-<昵称>-<mac6>。空输入或 bleinit 会恢复默认名。",
            (Lang::En, "alias_help_detail") => "1-8 lowercase letters or digits, no dashes",
            (Lang::Zh, "alias_help_detail") => "1-8 位小写字母或数字，不能含 -",
            (Lang::En, "alias_new_name") => "Saved BLE name:",
            (Lang::Zh, "alias_new_name") => "已保存的 BLE 名称：",
            (Lang::En, "alias_reconnect_hint") => "The current BLE name is unchanged. Restart the device (or the BLE service) to apply it.",
            (Lang::Zh, "alias_reconnect_hint") => "当前蓝牙名不会立刻改变。请重启设备后再扫描；机载电脑上重启服务即可。",
